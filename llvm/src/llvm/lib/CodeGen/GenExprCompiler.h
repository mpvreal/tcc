...
class GenExprTree {
  enum NodeType {
    OPERATION, VARIABLE, INT_VALUE, FLOAT_VALUE, TERN_RETURN
  } tag;
  ...
  union ReturnValue {
    ...
    ReturnValue(double toReal) : toReal(toReal) {};
    ReturnValue(bool toBool) : toBool(toBool) {};
  };

  // ReturnValue internalEvaluate();
...
class GenExprCompiler {
  ...
  std::unique_ptr<GenExprTree> unary(enum ExprToken token, const std::string& id);
  std::unique_ptr<GenExprTree> binaryOrTernary(enum ExprToken token);
